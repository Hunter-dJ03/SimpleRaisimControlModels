...
private:
	...
	void controlCommands()
	{
		...
		for (size_t leg = 0; leg < 4; ++leg)
		{
			...
			if (stepTimer[leg] < stepDuration/2) // Swing phase
			{
			...
			{
				desired_position(0) = footPositionInit[leg](0) + forwardStepLength * (1.0/2.0 - ((stepTimer[leg] - stepDuration/2) / (stepDuration - stepDuration/2))); 
				desired_position(1) = footPositionInit[leg](1) + sideStepLength * (1.0/2.0 - ((stepTimer[leg] - stepDuration/2) / (stepDuration - stepDuration/2))); 
				desired_position(2) = footPositionInit[leg](2);

				desired_velocity(0) = - (2.0 * forwardStepLength) / (1.0 * stepDuration);
				desired_velocity(1) = - (2.0 * sideStepLength) / (1.0 * stepDuration);
	...
	double sideWalkOffset[4] = {-sideStepLength / 2.0, sideStepLength / 2.0, - sideStepLength / 2.0, sideStepLength / 2.0};
	double stepTimer[4] = {stepDuration*(0.0/4.0), stepDuration*(2.0/4.0), stepDuration*(0.0/4.0), stepDuration*(2.0/4.0)};
